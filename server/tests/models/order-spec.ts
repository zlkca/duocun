...
describe('getOrdersToAddGroupDiscount with missing field', () => {
  it('should return orders to be updated', () => {
    ...
    const orders: any[] = [
      ...
      { id: 'e', clientId: 'b', groupDiscount: 0, total: 3 }, // take
      { id: 'f', clientId: 'c', groupDiscount: 0, total: 2 }, // take
      { id: 'g', groupDiscount: 0, total: 2 },
      { id: 'h', clientId: 'c', groupDiscount: 2 },
      { id: 'i', clientId: 'd', groupDiscount: 0, total: 3 }, // take
      { id: 'j', price: 5 },
      { id: 'k', clientId: 'd', groupDiscount: 0 },
    ];

    const a = c.getOrdersToAddGroupDiscount(orders, 2);
...
describe('getOrdersToAddGroupDiscount with empty', () => {
  it('should return empty Array', () => {
    const db = new DB();
    const c = new Order(db);

    const orders: any[] = [];
    const a = c.getOrdersToAddGroupDiscount(orders, 3);
...
describe('getOrdersToRemoveGroupDiscount, still be eligible and removed an order without discount', () => {
  it('should return orders to be updated', () => {
    ...
    const orders: any[] = [
      // { id: 'a', clientId: 'a', groupDiscount: 0, total: 1 },
      { id: 'b', clientId: 'a', groupDiscount: 2, total: 1 },
      { id: 'c', clientId: 'a', groupDiscount: 0, total: 1 },
      { id: 'd', clientId: 'b', groupDiscount: 2, total: 1 }, // take
      { id: 'e', clientId: 'b', groupDiscount: 0, total: 3 }, // take
      { id: 'f', clientId: 'c', groupDiscount: 0, total: 2 }, // take
      { id: 'g', groupDiscount: 0, total: 2 },
      { id: 'h', clientId: 'c', groupDiscount: 2, total: 4 },
      { id: 'i', clientId: 'd', groupDiscount: 0, total: 3 }, // take
      { id: 'j', price: 5 },
      { id: 'k', clientId: 'd', groupDiscount: 2 },
    ];
...
describe('getOrdersToRemoveGroupDiscount, still be eligible and removed an order with discount', () => {
  it('should return orders to be updated', () => {
    ...
    const orders: any[] = [
      { id: 'a', clientId: 'a', groupDiscount: 0, total: 1 },
      { id: 'b', clientId: 'a', groupDiscount: 0, total: 2 },
      // { id: 'c', clientId: 'a', groupDiscount: 2, total: 3 },
      { id: 'c', clientId: 'b', groupDiscount: 0, total: 1 },
      { id: 'c', clientId: 'b', groupDiscount: 2, total: 1 },
    ];
...
describe('getOrdersToRemoveGroupDiscount, not eligible and left 2 other orders', () => {
  it('should return orders to be updated', () => {
    ...
    const orders: any[] = [
      // { id: 'a', clientId: 'a', groupDiscount: 2, total: 1 },
      { id: 'c', clientId: 'b', groupDiscount: 0, total: 1 },
      { id: 'c', clientId: 'b', groupDiscount: 2, total: 1 },
    ];
...
describe('addGroupDiscountForOrders with 2nd groupDiscount 2', () => {
  ...
  it('should update orders with group discount', (done) => {
    ...
    oo.addGroupDiscountForOrders(order.clientId, ors).then((orderUpdates: any[]) => {
      const cIds: string[] = [];
      orders.map((o: any) => {
...
describe('addGroupDiscountForOrders with 2nd 0 groupDiscount', () => {
  const db: any = new DB();
  const cfg: any = new Config();
...
// addGroupDiscountForOrders(clientId: string, date: string, address: string, cb?: any)
describe('addGroupDiscountForOrders with 2nd 1 groupDiscount', () => {
  const db: any = new DB();
  const cfg: any = new Config();
  let oo: Order;
  let ao: Account;
  ...
  it('should update orders with group discount', (done) => {
    const order: any = ors[0];
    oo.addGroupDiscountForOrders(order.clientId, ors).then((x: any) => {
      const cIds: string[] = [];
      orders.map((o: any) => {
        cIds.push(o.clientId);
      });
...
describe('eligibleForGroupDiscount with 2 orders different accounts', () => {
      ...
      ao.insertMany(accounts).then((rs: any[]) => {
        ...
        orders = [
          { mode: 'test', clientId: rs[0].id.toString(), clientName: rs[0].username, address:'abc', 
            delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 0, total:8.5 },
          { mode: 'test', clientId: rs[1].id.toString(), clientName: rs[1].username, address:'abc',
            delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 1, total:9.5 },
        ];
  ...
  it('should be eligible for the group discount', (done) => {
    ...
    oo.eligibleForGroupDiscount(o1.clientId, o1.delivered, o1.address, (bEligible1: boolean) => {
      expect(bEligible1).to.equal(true);
      
      oo.eligibleForGroupDiscount(o2.clientId, o2.delivered, o2.address, (bEligible1: boolean) => {
...
describe('eligibleForGroupDiscount with 1 orders different accounts', () => {
  ...
  it('should be eligible for the group discount', (done) => {
    const o1: any = ors[0];
    oo.eligibleForGroupDiscount(acs[1].id, o1.delivered, o1.address, (bEligible1: boolean) => {
      expect(bEligible1).to.equal(true);
      done();
    });
  });
  ...
  before(function(done) {
    db.init(cfg.DATABASE).then((dbClient: any) => {
      ...
      ao.insertMany(accounts).then((rs: any[]) => {
        ...
        orders = [
          { mode: 'test', clientId: rs[0].id.toString(), clientName: rs[0].username, address:'abc', 
            delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 0, total:8.5 }, // new insert
          { mode: 'test', clientId: rs[0].id.toString(), clientName: rs[0].username, address:'abc',
          delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 0, total:9.5 },
          { mode: 'test', clientId: rs[0].id.toString(), clientName: rs[0].username, address:'abc',
          delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 0, total:7.5 },
        ];

        oo.deleteMany({mode:'test'}).then(() => {
          oo.insertMany(orders).then((os: any[]) => {
            ors = os;
            done();
          });
...
describe('eligibleForGroupDiscount with 2 orders of the same accounts', () => {
  const db: any = new DB();
  const cfg: any = new Config();
  let oo: Order;
  let ao: Account;
  ...
  before(function(done) {
    db.init(cfg.DATABASE).then((dbClient: any) => {
      ...
      ao.insertMany(accounts).then((rs: any[]) => {
        ...
        orders = [
          { mode: 'test', clientId: rs[0].id.toString(), clientName: rs[0].username, address:'abc', delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 0, total:8.5 },
          { mode: 'test', clientId: rs[0].id.toString(), clientName: rs[0].username, address:'abc', delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 0, total:9.5 },
          { mode: 'test', clientId: rs[0].id.toString(), clientName: rs[0].username, address:'abc', delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 0, total:7.5 },
        ];
  ...
  before(function(done) {
    db.init(cfg.DATABASE).then((dbClient: any) => {
      ...
      ao.insertMany(accounts).then((rs: any[]) => {
        ...
        orders = [
          { mode: 'test', clientId: rs[0].id.toString(), clientName: rs[0].username, address:'abc', delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 0, total:8.5 },
          { mode: 'test', clientId: rs[0].id.toString(), clientName: rs[0].username, address:'abc', delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 2, total:9.5 },
          { mode: 'test', clientId: rs[1].id.toString(), clientName: rs[1].username, address:'abc', delivered: '2019-04-23T15:45:00.000Z', groupDiscount: 2, total:7.5 },
        ];
