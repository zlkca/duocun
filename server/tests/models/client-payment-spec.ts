...
describe('processAfterAddOrder with existing order of other client', () => {
  const db: DB = new DB();
  ...
  it('should update orders with group discount', (done) => {
    ...
    cpo.processAfterAddOrder(orderId, 0).then((x: any) => {
      const cIds: string[] = [];
      orders.map((o: any) => {
        cIds.push(o.clientId);
      });
      oo.find({clientId: {$in: cIds}}).then((o1s: any[]) => {
        ...
        cbo.find({accountId: {$in: cIds}}).then((cbs: any[]) => {
          ...
          expect(mbMe.amount).to.equal(-10.5);
...
describe('processAfterAddOrder with no order of other client', () => {
  const db: any = new DB();
  const cfg: any = new Config();
  let clientConnection: any = null;
  let oo: Order;
  ...
  it('should update orders with group discount', (done) => {
    const clientId = ors[0].clientId;
    cpo.processAfterAddOrder(clientId, 0).then( (x: any) => {
      const cIds: string[] = [];
      orders.map((o: any) => {
        cIds.push(o.clientId);
      });
